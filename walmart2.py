...
class WalmartSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        products = []
        products_to_fetch = data['results']['entities']['productsToFetch']
        for product in products_to_fetch:
            new_json = {
                "product_id": product['product_id'],
                "sku_id": product['sku_id']
            }
            products.append(new_json)
        ...
        more_products_api = "https://www.walmart.ca/api/bsp/fetch-products?experience=grocery"

        yield scrapy.Request(more_products_api, method="POST", body=json.dumps(payload), headers=self.headers,
                             callback=self.parse, meta={"seen": True})
